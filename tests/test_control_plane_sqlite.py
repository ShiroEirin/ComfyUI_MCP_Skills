...
def test_initialize_creates_versioned_control_plane_schema(tmp_path: Path) -> None:
    ...
    assert applied == [
        ...
        (13, "g2-runtime-restart-execution", 64, 1, 0, 1),
    ]
...
def test_initialize_is_idempotent_and_detects_checksum_drift(tmp_path: Path) -> None:
    ...
    with sqlite3.connect(database) as connection:
        assert connection.execute("SELECT count(*) FROM schema_migrations").fetchone() == (13,)
        connection.execute(
...
def test_forward_migration_refreshes_fingerprints_and_multilevel_rollback(
    ...
    monkeypatch.setattr(
        ...
        "_MIGRATIONS",
        (*control_plane_module._MIGRATIONS, migration_v14),
    )
    store.initialize()
