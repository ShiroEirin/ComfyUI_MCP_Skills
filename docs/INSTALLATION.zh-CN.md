...
- **速率按源 IP，并发按主体**：速率桶在认证前按客户端源 IP 消费，并发许可在认证后按主体归因。反向代理/同主机多 worker 部署下所有客户端呈现为代理 IP，共享同一速率桶——单一租户的突发流量可对其他租户造成速率饥饿；这是刻意的保守语义（防止未认证流量绕过限流），需要每主体速率配额时应在反向代理层实现。
