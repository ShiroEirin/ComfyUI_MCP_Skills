...
3. **`change.plan` 校验增强 + 失败引导**：plan 阶段对照服务器 object_info 校验 class_type 存在性、输入字段存在性、枚举合法性；失败错误带修复路径（hint: 用 `comfyui.node.describe KSampler` 查看正确输入）；可选 `suggested_queries` 字段。
